...
static DbManager dbm("rest", "orix");

bool ImportsValidate(const json& req) {
...
void ChangeSize(const std::string& id, int delta, const std::string& date) {
  std::string parent = db::GetParent(dbm, id);
  if (!parent.empty())
    ChangeSize(parent, delta, date);
...
void ImportsHandler(const Request& req, Reply& rep) {
  if (!ImportsValidate(req.body)) {
...
int DeleteIter(const std::string& id) {
  int deleted_size = 0;
  for (const std::string& child_id : db::GetChildren(dbm, id))
    deleted_size += DeleteIter(child_id);
...
void DeleteHandler(const Request& req, Reply& rep) {
  ...
  std::string parent = db::GetParent(dbm, id);
  int deleted_size = DeleteIter(id);
  if (!parent.empty())
    ChangeSize(parent, -deleted_size, req.params.at("date"));

  rep.status = Reply::ok;
}

json NodesIter(const std::string& id) {
  json item = db::GetItem(dbm, id);
  if (item["type"].get<std::string>() == "FOLDER") {
    item["children"] = json::value_t::array;
    for (const std::string& child : db::GetChildren(dbm, id))
      item["children"].push_back(NodesIter(child));
  } else {
...
void NodesHandler(const Request& req, Reply& rep) {
  std::string id = 
    req.uri.substr(req.uri.find_first_of("/") + 1);
  ...
  json item = NodesIter(id);
  rep.content = item.dump(2, ' ');
  rep.status = Reply::ok;
}

void UpdatesHandler(const Request& req, Reply& rep) {
  if (req.params.count("date") == 0
    || !utils::IsDateValid(req.params.at("date"))) {
    rep = Reply::StockReply(Reply::bad_request);
    return;
...
void HistoryHandler(const Request& req, Reply& rep) {
...
int main(int argc, char* argv[]) {
  try {
    ...
    request_handler.SetHandler("imports", ImportsHandler);
    request_handler.SetHandler("delete", DeleteHandler);
    request_handler.SetHandler("nodes", NodesHandler);
    request_handler.SetHandler("updates", UpdatesHandler);
    request_handler.SetHandler("node", HistoryHandler);
